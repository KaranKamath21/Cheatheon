...
PAGE_LIMIT = 100  # This is now set to 10 pages
...
def get_all_submissions(contest_slug: str, questions: List[dict]) -> None:
    ...
    # Wrap the page range in tqdm for progress tracking
    for i in tqdm.tqdm(range(1, PAGE_LIMIT + 1), desc="Processing pages"):
        ...
        # Wrap user submissions processing in tqdm
        for user_submissions, user in tqdm.tqdm(zip(response["submissions"], response["total_rank"]),
                                           desc=f"Processing users on page {i}", leave=False):
